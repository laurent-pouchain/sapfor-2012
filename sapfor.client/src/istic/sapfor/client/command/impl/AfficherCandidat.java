...
public class AfficherCandidat implements ICommand {
	...
	public Boolean execute(ICommandContext ctx) {
		...
		Collection<Long> idCand = client.getIdCandidat((long) idUv,
				EtatCandidatureDTO.inscrit);
		Collection<Long> idRet = client.getIdCandidat((long) idUv,
				EtatCandidatureDTO.retenu);
		Collection<Long> idNonRet = client.getIdCandidat((long) idUv,
				EtatCandidatureDTO.nonRetenu);
		Collection<Long> idAtt = client.getIdCandidat((long) idUv,
				EtatCandidatureDTO.listeAttente);

		// test avec une liste non vide pour l'IHM
		// on ne peu pas tester si les onglets son pise en compte ou si il faut
		// un listener
	...
	void retenu(String idTemp, Collection<Long> idRet,
			Collection<Long> idNonRet, Collection<Long> idAtt) {
		if (idRet == null) {
	...
	void nonRetenu(String idTemp, Collection<Long> idNonRet,
			Collection<Long> idAtt) {
		if (idNonRet == null) {
